export default async function (ctx) {
  ...
  // Body stream can only be consumed once, read as text first for later substring operations
  const rawBody = await ctx.response.text();
  let body = JSON.parse(rawBody);

  if (method !== "GET") {
    console.log(url);
    ctx.notify({ title: noticeTitle, body: `method错误: ${method}` });
  }
  ...
  if (url.includes("api.zhihu.com/commercial_api/real_time_launch_v2")) {
    ...
    if (!body.launch) {
      ...
      ctx.notify({ title: noticeTitle, subtitle: ctx.script.name, body: "launch字段为空" });
    }
    ...
    if (!dataArr) {
      console.log(`body:${rawBody}`);
      ctx.notify({ title: noticeTitle, subtitle: "知乎推荐", body: "data字段为空" });
    } else {
      body.data = dataArr.filter((item) => {
        if (item.extra?.type === "zvideo") {
          ...
          if (!videoID) {
            ...
            console.log(
              `videoID处理成功,原始:${item.common_card.feed_content.video.id},修改为:${videoID}`
            );
          ...
          if (!videoID) {
            ...
            console.log(
              `market_card-videoID处理成功,原始:${item.fields.body.video.id},修改为:${videoID}`
            );
            item.fields.body.video.id = videoID;
          }
        } else if (item.common_card?.feed_content?.video?.id) {
          let search = '"feed_content":{"video":{"id":';
          let str = rawBody.substring(rawBody.indexOf(search) + search.length);
          let videoID = str.substring(0, str.indexOf(","));
          console.log(
            `其他-videoID处理成功,原始:${item.common_card.feed_content.video.id},修改为:${videoID}`
          );
          item.common_card.feed_content.video.id = videoID;
        }
    ...
    url.includes("api.zhihu.com/questions") ||
    url.includes("api.zhihu.com/v4/questions")
  ) {
    ...
    if (!body.paging || !body.data) {
      console.log(`body:${rawBody}`);
      ctx.notify({ title: noticeTitle, subtitle: "知乎回答下广告", body: "paging/data字段为空" });
    } else {
      body.paging = null;
      body.data = null;
      console.log("成功");
    ...
    if (!body.ad_info) {
      ...
      ctx.notify({
        title: noticeTitle,
        subtitle: ctx.script.name,
        body: "articles-ad_info字段为undefined",
      });
    } else {
      body.ad_info = null;
      console.log("成功");
    ...
    ctx.notify({ title: noticeTitle, body: `路径匹配错误: ${url}` });
  }
